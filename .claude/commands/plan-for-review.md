...
# Pawsitive Detectives — draft a plan for cross-AI review
...
approach would cost more than one implementation cycle to undo. Skip it for one-line, single-file, or
mechanical changes — tell the user to proceed directly. Reach for it when the task touches multiple areas
of `app.js`, has real architectural implications (e.g. changing `state`'s shape, splitting the single-file
structure), or scope is genuinely contested/unclear. For a genuinely large effort, split into linked plans
rather than one unreviewable mega-doc.
...
Research whatever the plan touches (read the relevant parts of `app.js`; check `AGENTS.md`/
`docs/TASK_BOARD.md`/`DECISIONS.md` for constraints) and work out a real plan:
- the concrete goal, and how you'd know it's done
- the proposed approach, ordered steps, naming concrete functions/consts (they don't rot like line numbers)
- alternatives considered and why not chosen
- what's explicitly out of scope
- risks and open questions
- which files/areas it touches

Assume the reviewer has **no access to this repo** — quote or paraphrase any constraint inline; "see
AGENTS.md" is useless to them. Distinguish facts you verified from things you're assuming.

**Never inline secrets/credentials** — paraphrase the existence of a constraint, not its value.
...
## Files involved
- <path — what changes and why, named by function/const, not line number>
...
## Verification
<how you'll prove the goal was met — for this repo that's manual verification per `docs/HOW-TO-WORK.md`
(no automated suite exists), plus any task-specific check like a save-migration test.>
...
Then, as a separate question, ask whether to commit it (docs-only, no code/rules change) — fine to leave
uncommitted if it's just being pasted elsewhere. Pushing is a further, separate ask.
...
- Stop and ask the user before acting on anything touching secrets, contradicting an existing
  `DECISIONS.md` entry, or reflecting reviewer disagreement.
