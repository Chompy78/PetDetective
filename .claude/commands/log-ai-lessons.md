...
# Pawsitive Detectives — mine a session for cross-project lessons
...
`inbox/`, commit, or push without approval, same convention as `/close-session`'s item 8 (which this
...
For each genuinely new, generalizable lesson (not specific to this game or its stack), draft:
